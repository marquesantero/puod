...
import { getAuthProfilesForClient, type AuthProfileListResponse } from "@/lib/authProfileApi";
import { Can } from "@/components/auth/Can";
import { Trash2, UserPlus, Search, Check, Building2 } from "lucide-react";
...
import {
  ...
  updateUserCompanyAvailability,
  deleteUser,
  IdentitySource,
  type IdentityUserResult,
} from "@/lib/securityApi";

interface ClientUserManagementTabProps {
  clientId: number;
  isPlatformClient?: boolean;
}

export function ClientUserManagementTab({ clientId, isPlatformClient = false }: ClientUserManagementTabProps) {
  const { t } = useI18n();
  ...
  // Company availability for non-Platform clients
  const [companies, setCompanies] = useState<CompanyOption[]>([]);
  const [selectedCompanyIds, setSelectedCompanyIds] = useState<number[]>([]);

  // Edit companies dialog
  const [isEditCompaniesDialogOpen, setIsEditCompaniesDialogOpen] = useState(false);
  const [editingUser, setEditingUser] = useState<IdentityUserResult | null>(null);
  const [editCompanyIds, setEditCompanyIds] = useState<number[]>([]);
  const [savingCompanies, setSavingCompanies] = useState(false);
  const [userToEditCompanies, setUserToEditCompanies] = useState<IdentityUserResult | null>(null);
  ...
  useEffect(() => {
    // Platform client only uses Local authentication
    if (!isPlatformClient) {
      loadAuthProfiles();
      loadCompanies();
    }
    loadUsers();
  }, [clientId, isPlatformClient]);

  const loadAuthProfiles = async () => {
    try {
      const profiles = await getAuthProfilesForClient(clientId);
      setAuthProfiles(profiles.filter((p: any) => p.isActive));
    } catch (e) {
      console.error(e);
    }
  };
  ...
  const loadCompanies = async () => {
    try {
      ...
      console.log("ClientsData:", clientsData, "ClientId:", clientId);
      const client = clientsData.find(c => c.id === clientId);
      console.log("Found client:", client);
      console.log("client.companies:", client?.companies);
      if (client && client.companies) {
        const mappedCompanies = client.companies.map(c => ({
          id: c.id,
          name: c.name,
          slug: c.slug
        }));
        ...
        console.log("No companies found for client");
      }
    } catch (e) {
  ...
  const loadUsers = async () => {
    ...
    try {
      const data = await getUsersForClient(clientId);
      setUsers(data);
    } catch (e) {
      console.error(e);
    } finally {
  ...
  const handleImportSearch = async () => {
    ...
    try {
      ...
      const results = await searchUsers(importSearchTerm, sourceEnum, profileId ? String(profileId) : undefined);
      setImportSearchResults(results.map(r => ({
          ...r,
  ...
  const handleImport = async (user: IdentityUserResult) => {
    // For non-Platform clients, validate company selection
    if (!isPlatformClient && selectedCompanyIds.length === 0) {
        showToast(t("selectAtLeastOneCompany") || "Please select at least one company", { variant: "destructive" });
        return;
    }
    ...
    try {
        await importUser({
            ...
            source: user.source,
            isClientLevel: true,
            companyIds: isPlatformClient ? undefined : selectedCompanyIds.map(String)
        });
        ...
        if (error.response?.status === 409 && error.response?.data) {
            const { existsAt, location } = error.response.data;

            if (existsAt === "client") {
                errorMessage = `${t("usersAlreadyExistsAtClient")} (${location})`;
            } else if (existsAt === "company") {
                errorMessage = `${t("usersAlreadyExistsAtCompany")} (${location})`;
            } else {
                errorMessage = t("usersAlreadyExistsAtSystem");
            }
        }
  ...
  const handleCreateLocalUser = async (e: React.FormEvent) => {
    ...
    // For non-Platform clients, validate company selection
    if (!isPlatformClient && selectedCompanyIds.length === 0) {
      setCompanySelectionError(t("selectAtLeastOneCompany") || "Please select at least one company");
      return;
    }
  ...
  const handleEditCompanies = async (user: IdentityUserResult) => {
      setEditingUser(user);

      try {
          const availability = await getUserCompanyAvailability(user.id, clientId);
          const availableIds = availability.filter(c => c.isAvailable).map(c => c.companyId);
          setEditCompanyIds(availableIds);
  ...
  const requestEditCompanies = (e: React.MouseEvent, user: IdentityUserResult) => {
      e.stopPropagation();
      setUserToEditCompanies(user);
  };

  const confirmEditCompanies = () => {
      if (!userToEditCompanies) return;
      handleEditCompanies(userToEditCompanies);
      setUserToEditCompanies(null);
  };
  ...
  const handleSaveCompanies = async () => {
      ...
      try {
          await updateUserCompanyAvailability(editingUser.id, clientId, editCompanyIds);
  ...
  const handleDelete = async (userId: number) => {
      ...
      // Platform client: ensure at least one active system_admin remains
      if (isPlatformClient) {
          const activeUsers = users.filter(u => u.isActive && u.id !== userId);
          if (activeUsers.length === 0) {
              showToast(t("platformMustHaveOneAdmin"), { variant: "destructive" });
              return;
          }
      }
    ...
    <div className="space-y-8 pt-6 animate-in fade-in duration-500">
      <div className="flex flex-col gap-6 bg-accent/10 p-6 rounded-3xl border border-border/50 shadow-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div className="flex-1 max-w-md space-y-2">
              ...
              <div className="relative">
                ...
                <Input placeholder={t("usersFilterPlaceholder")} value={listSearchTerm} onChange={(e) => setListSearchTerm(e.target.value)} className="bg-background pl-10 h-11 rounded-xl" />
              </div>
          </div>
        ...
        <div className="flex flex-wrap items-center gap-6 pt-2">
            ...
            <>
              ...
              <div className="flex flex-wrap gap-3">
                {[IdentitySource.Local, IdentitySource.WindowsAd, IdentitySource.AzureAd].map(source => (
                  <label key={source} className={`flex items-center gap-2 px-3 py-1.5 rounded-full border transition-all cursor-pointer select-none ${sourceFilters.has(source) ? 'bg-primary/10 border-primary text-primary' : 'bg-background border-border text-muted-foreground hover:border-primary/30'}`}>
                    <input type="checkbox" className="hidden" checked={sourceFilters.has(source)} onChange={() => {
          ...
          <label className={`flex items-center gap-2 px-3 py-1.5 rounded-full border transition-all cursor-pointer select-none ${showInactive ? 'bg-destructive/10 border-destructive text-destructive' : 'bg-background border-border text-muted-foreground hover:border-destructive/30'}`}>
          ...
          {isPlatformClient && (
            <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 text-amber-900 dark:text-amber-100">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
              <span className="text-xs font-bold">{t("platformAllUsersAreSystemAdmin")}</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {loadingUsers ? Array.from({ length: 4 }).map((_, i) => <div key={i} className="h-32 rounded-3xl bg-accent/10 animate-pulse border border-dashed" />) : 
         filteredUsers.length === 0 ? <div className="col-span-full py-20 text-center border-2 border-dashed rounded-3xl text-muted-foreground italic bg-accent/5"><p className="text-lg font-medium">{t("usersListEmpty")}</p></div> :
         filteredUsers.map(user => (
            <Card key={user.id} className={`group relative overflow-hidden cursor-pointer hover:border-primary/50 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 rounded-3xl ${!user.isActive ? 'opacity-60 grayscale' : ''}`} onClick={() => navigate(`/users/${user.id}?scope=client&clientId=${clientId}`)}>
                ...
                <CardContent className="p-6">
                    ...
                    <div className="flex items-center justify-between mt-2 pt-4 border-t border-border/50">
                        <span className={`px-2.5 py-0.5 rounded-lg text-[10px] font-black uppercase tracking-wider border ${getSourceBadgeColor(user.source)}`}>{getSourceName(user.source)}</span>
                        <div className="flex items-center gap-1">
                            {!isPlatformClient && (
                                <Button type="button" variant="ghost" size="sm" className="h-8 w-8 p-0 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity hover:bg-primary/10 hover:text-primary" onClick={(e) => requestEditCompanies(e, user)}>
                                    <Building2 className="w-4 h-4" />
                                </Button>
                            )}
                            <Button type="button" variant="ghost" size="sm" className="h-8 w-8 p-0 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity hover:bg-destructive/10 hover:text-destructive" onClick={(e) => requestDelete(e, user)}>
      ...
      <ConfirmDialog
        open={!!userToEditCompanies}
        onOpenChange={(open) => {
          if (!open) setUserToEditCompanies(null);
        }}
        title={t("confirm")}
        description={t("editConfirm")}
        confirmLabel={t("edit")}
        cancelLabel={t("cancel")}
        onConfirm={confirmEditCompanies}
      />
      <ConfirmDialog
        open={!!userToDelete}
        onOpenChange={(open) => {
      ...
      <Dialog open={isAddDialogOpen} onOpenChange={handleDialogClose}>
          <DialogContent className="max-w-4xl max-h-[85vh] overflow-y-auto rounded-3xl" aria-describedby="dialog-description">
            <DialogHeader>
              <DialogTitle className="text-2xl font-black tracking-tight">
                {importSource === "Local" ? t("usersCreateLocalTitle") || "Create Local User" : t("usersAddTitle")}
              </DialogTitle>
            ...
            <div className="space-y-6 py-4">
                {isPlatformClient && (
                  <div className="flex items-start gap-3 p-4 rounded-2xl bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800 text-amber-900 dark:text-amber-100">
                    ...
                    <p className="text-sm font-medium">{t("platformUsersAreSystemAdmin")}</p>
                  </div>
                )}

                <div className="bg-accent/10 p-6 rounded-2xl border space-y-4">
                  {!isPlatformClient && (
                    <div className="space-y-2">
                        <Label className="text-[10px] font-black uppercase tracking-widest">{t("usersSource")}</Label>
                        <Select value={importSource} onValueChange={setImportSource}>
                            <SelectTrigger className="bg-background rounded-xl h-11"><SelectValue /></SelectTrigger>
                            <SelectContent>
                                <SelectItem value="Local">{t("localAuth")}</SelectItem>
                                {authProfiles.map(p => <SelectItem key={p.id} value={p.id.toString()}>{p.name}</SelectItem>)}
                            </SelectContent>
                        </Select>
                    </div>
                  )}
                  ...
                  {importSource === "Local" ? (
                    <>
                      {!isPlatformClient && companies.length > 0 && (
                        <div className="pt-4 border-t space-y-2">
                          <CompanySelector
                            ...
                            onChange={(ids) => {
                                ...
                                if (companySelectionError) setCompanySelectionError("");
                            }}
                          />
                          {companySelectionError && (
                            <p className="text-xs text-destructive font-medium">{companySelectionError}</p>
                          )}
                        </div>
                      )}
                    ...
                    <>
                      ...
                      <div className="flex gap-2 pt-4">
                        <Input className="bg-background h-11 rounded-xl" value={importSearchTerm} onChange={e => setImportSearchTerm(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleImportSearch()} placeholder={t("usersSearchPlaceholder")} />
                        <Button type="button" onClick={handleImportSearch} disabled={searching} className="h-11 rounded-xl px-6">
                          {searching ? <span className="animate-spin rounded-full h-4 w-4 border-b-2 border-white" /> : <Search className="w-4 h-4" />}
                        </Button>
                      </div>
                    </>
                  )}
                ...
                {importSource !== "Local" && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {importSearchResults.map(u => (
                      <Card key={u.id} className="overflow-hidden border-border/50 rounded-2xl">
                        <CardContent className="p-4 flex items-center justify-between gap-3">
                          <div className="flex items-center gap-3 min-w-0">
                            <div className="h-10 w-10 rounded-xl bg-primary/10 text-primary flex items-center justify-center font-bold shrink-0">{u.displayName.substring(0,2).toUpperCase()}</div>
                            <div className="min-w-0"><p className="text-sm font-bold truncate">{u.displayName}</p><p className="text-[10px] text-muted-foreground truncate">{u.username}</p></div>
                          </div>
                          <Button size="sm" variant={u.isImported ? "ghost" : "default"} disabled={u.isImported || importing === u.id} onClick={() => handleImport(u)} className="rounded-lg h-8 px-3 text-xs">
                            {u.isImported ? <Check className="w-4 h-4 text-emerald-500" /> : importing === u.id ? "..." : t("usersImport")}
                          </Button>
                        </CardContent>
                      </Card>
                    ))}
      ...
      <Dialog open={isEditCompaniesDialogOpen} onOpenChange={setIsEditCompaniesDialogOpen}>
          <DialogContent className="max-w-2xl rounded-3xl">
            <DialogHeader>
                <DialogTitle className="text-2xl font-black tracking-tight">
                    {t("usersEditCompanies") || "Edit Companies"}
                </DialogTitle>
                {editingUser && (
            ...
            <div className="space-y-6 py-4">
                {companies.length > 0 && (
                    <CompanySelector
                        companies={companies}
                        selectedCompanyIds={editCompanyIds}
                ...
                <div className="flex justify-end gap-3 pt-4">
                    <Button
                        type="button"
                        variant="outline"
                        onClick={() => setIsEditCompaniesDialogOpen(false)}
                    ...
                    <Button
                        ...
                        className="rounded-xl"
                    >
                        {savingCompanies ? t("loading") : t("save")}
                    </Button>
                </div>
            </div>
          </DialogContent>
